...
TOTAL_BUDGET_S = 540  # 9 minutes
STAGE1_BUDGET_S = 300  # 5 minutes limit for Stage 1
...
def run_stage_training(
    ...
    if is_stage2:
        ...
        for item in split_res["train"]:
            ...
            if not (fname.startswith("trashnet_") or fname.startswith("taco_")):
    ...
    model, train_tf, eval_tf = get_model_and_transforms("mobilenet_v3_large", num_classes, pretrained=True, is_stage2=is_stage2)
...
def main():
    ...
    # Stage 1
    stage1_metrics = run_stage_training(
        stage_name="Stage 1 (O vs R)",
        split_path="outputs/stage1_split.json",
        classes=["O", "R"],
        global_start=global_start,
        time_limit_s=STAGE1_BUDGET_S,
        output_path="outputs/stage1_mobilenet.pt",
    ...
    stage2_metrics = run_stage_training(
        ...
        output_path="outputs/stage2_material_mobilenet.pt",
        finetune_if_below=0.92,
        unfreeze_layer="features.14"
    )
    ...
    if os.path.exists(report_path):
        ...
        # Rotate previous
        if "stage1_mobilenet" in report:
            report["_previous_stage1_mobilenet"] = report["stage1_mobilenet"]
        if "stage2_mobilenet" in report:
            report["_previous_stage2_mobilenet"] = report["stage2_mobilenet"]
            
        # Deprecate resnet
        if "stage2_resnet50" in report:
            report["_DEPRECATED_stage2_resnet50"] = report.pop("stage2_resnet50")
            
        report["stage1_mobilenet"] = stage1_metrics
        report["stage2_mobilenet"] = stage2_metrics
