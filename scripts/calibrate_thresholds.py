...
STAGE1_CKPT = "outputs/stage1_mobilenet_BELTCOMPOSITE.pt"
STAGE2_CKPT = "outputs/stage2_material_mobilenet_BELTCOMPOSITE.pt"
...
# Same values the live webapp's PRESET_THRESHOLDS/cascaded_classify_fn use
# today, included in the sweep so the report can show a direct "old value
# vs. new recommendation" comparison.
CURRENT_STAGE1_CUTOFF = 0.65
CURRENT_STAGE2_THRESHOLD = 0.50
...
def main():
    ...
    print("Loading Stage 1 (BELTCOMPOSITE)...")
    model1, transform1 = load_model(STAGE1_CKPT, 2, device)
    print("Loading Stage 2 (BELTCOMPOSITE)...")
    ...
    with open("outputs/threshold_calibration_BELTCOMPOSITE.json", "w") as f:
        json.dump({
    ...
    print("\nWrote outputs/threshold_calibration_BELTCOMPOSITE.json")
